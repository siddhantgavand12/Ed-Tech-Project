...
exports.signUp = async (req,res) =>{
    try{
        ...
        if(password != confirmPassword){
            return res.status(400).json({
                success: false,
                message: "Password and confirmpassword value does not matched, Please try agian"
            })
        }

        //check user already exist or not
        const existingUswer = await User.findOne({email});
...
// change password
expors.changePassword = async (req, res) =>{
    // get data from req body
